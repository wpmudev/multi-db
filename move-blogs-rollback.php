...
///DB Settings
$target_dbname = "target_db";  //New single DB. It should be already created.
$multi_db_prefix = 'mdtest_';  //This is the prefix of the DB's you're moving your tables from.
...
$dbh_multi = mysql_connect( $dbhost, $dbuname, $dbpass );
if ( !$dbh_multi ) {
	...
	<body>
		<table align="center" class="stats">
            ...
            foreach($db_ids as $i => $hex){
                ...
                $result = mysql_query( 'SHOW TABLES FROM ' . $current_db_name, $dbh_multi );
                if ( !$result ) {
                ...
                while ( $row = mysql_fetch_row( $result ) ) {
                    ...
                    $dbh_target = mysql_connect( $dbhost, $dbuname, $dbpass ) or die( "Houston, we have a problem!<br>Database Error: " . mysql_error() );
                    mysql_select_db( $target_dbname, $dbh_target ) or die( "Houston, we have a problem!<br><b>Looks like you need to create your target DB '" . $target_dbname . "'. <br /> Database Error: " . mysql_error() );

                    $tableexists = mysql_num_rows( mysql_query( "SHOW TABLES LIKE '{$row[0]}'", $dbh_target ) );
                    ...
                    if ( $doing_rollback && !$tableexists ) {
                        mysql_query( "CREATE TABLE {$row[0]} LIKE {$current_db_name}.{$row[0]}", $dbh_target );
                        mysql_query( "INSERT INTO {$row[0]} SELECT * FROM {$current_db_name}.{$row[0]}", $dbh_target );
                    }
